#!/usr/bin/env python3
"""
Gold Query Generator for SQL Agent Testing
Creates complex SQL queries with expected outputs for evaluation
"""
...
class GoldQueryGenerator:
    ...
    def add_gold_query(self, query_id, question, query, difficulty, tags=None):
        """Add a gold query with its expected output"""
        print(f"Generating gold query {query_id}: {question[:50]}...")
        
        expected_output = self.execute_query(query)
        ...
        print(f"✅ Query {query_id} completed with {len(expected_output)} results")
    
    def generate_all_gold_queries(self):
        """Generate all gold queries for testing"""
        ...
        self.add_gold_query(
            ...
            """,
            "hard",
            ["join", "aggregation", "multi_table", "limit"]
        )
        ...
        self.add_gold_query(
            ...
            """,
            "enterprise",
            ["window_function", "rank", "percentile", "subquery"]
        )
        ...
        self.add_gold_query(
            ...
            """,
            "enterprise",
            ["cte", "window_function", "aggregation"]
        )
        ...
        self.add_gold_query(
            ...
            """,
            "medium",
            ["date_functions", "aggregation", "group_by"]
        )
        ...
        self.add_gold_query(
            ...
            """,
            "hard",
            ["join", "subquery", "case_statement", "aggregation"]
        )
        ...
        self.add_gold_query(
            ...
            """,
            "hard",
            ["join", "aggregation", "group_by", "customer_analysis"]
        )
        ...
        self.add_gold_query(
            ...
            """,
            "hard",
            ["conditional_aggregation", "case_statement", "group_by"]
        )
        ...
        self.add_gold_query(
            ...
            """,
            "enterprise",
            ["join", "window_function", "aggregation", "ranking"]
        )
        
        # 12. Time-based cohort analysis
        self.add_gold_query(
            "cohort_analysis",
            ...
            """,
            "enterprise",
            ["cte", "cohort_analysis", "date_functions", "aggregation"]
        )
    
    def save_gold_queries(self, filename="gold_queries.json"):
        """Save gold queries to JSON file"""
        with open(filename, 'w') as f:
            json.dump(self.gold_queries, f, indent=2, default=str)
        
        print(f"\n✅ Saved {len(self.gold_queries)} gold queries to {filename}")
        
        # Print summary
        difficulty_counts = {}
        for query in self.gold_queries:
        ...
        for diff, count in difficulty_counts.items():
            print(f"{diff:12}: {count:2} queries")
        print(f"{'Total':12}: {len(self.gold_queries):2} queries")
...
if __name__ == "__main__":
    generator = GoldQueryGenerator()
    generator.generate_all_gold_queries()
    generator.save_gold_queries()
    generator.close()

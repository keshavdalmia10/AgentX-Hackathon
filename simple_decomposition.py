#!/usr/bin/env python3
"""
Simple Query Decomposition and Comparison
"""
...
def compare_approaches(original_query: str, expected_results: list, query_id: str):
    ...
    try:
        ...
        original_score = original_result['scores']['overall']
        original_time = original_result['execution']['execution_time_ms']
        print(f"   Score: {original_score:.2%}")
        print(f"   Time: {original_time:.2f}ms")
        print(f"   Rows: {original_result['execution']['rows_returned']}")
    ...
    for i, subq in enumerate(subqueries):
        ...
        try:
            ...
            time = result['execution']['execution_time_ms']
...
def main():
    ...
    if results:
        ...
        if avg_decomposed > avg_original:
            print(f"🏆 DECOMPOSITION approach wins overall")
        else:
            print(f"🎯 ORIGINAL approach wins overall")
